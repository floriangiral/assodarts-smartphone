...
export const stripeCreateCheckout = onCall(
  ...
  async (request) => {
    ...
    const call = (
      await db.collection("payment_calls").doc(item.paymentCallId).get()
    ).data();
    const bank = (
      await db.collection("club_bank_accounts").doc(item.clubId).get()
    ).data();

    if (!bank?.stripeAccountId || bank.stripeStatus !== "verified") {
      throw new HttpsError(
        "failed-precondition",
        "This club cannot collect online payments yet",
      );
    }
