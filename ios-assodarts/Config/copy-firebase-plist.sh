...
# An empty plist leaves every Firebase value blank, which keeps the app without
# server access instead of failing at launch.
for plist in GoogleService-Info.plist FirebaseConfig.plist; do
    if [ -f "$source_dir/$plist" ]; then
        ...
        echo "warning: $plist not found in $source_dir; the app will start without server access"
